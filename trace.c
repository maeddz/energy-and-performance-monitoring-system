...
static void start_power_tracing(struct trace_array *tr);
static void periodic_update_power(void);
static struct battery_trace bt;
//static DECLARE_DELAYED_WORK(power_trace,read_power_data);
...
static void read_power_data(void){
    ...
    while (!stop_power_tracing){
		bt.psy->get_property(bt.psy,POWER_SUPPLY_PROP_CURRENT_NOW,&value);
		write_power_ringbuffer(value.strval);
		msleep(100);
	}

}
void start_power_tracing(struct trace_array *tr){
	// // // 
...
void periodic_update_power(struct trace_array *tr){
	int ret;
	power = kthread_run(start_power_tracing,tr, "power_data");
	if (IS_ERR(power))
		goto out_fail;
	return;
out_fail:
